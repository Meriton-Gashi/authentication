...
<div class="mainPage" style="text-align: center;">
    <fieldset>
    <form id="contactForm" method="POST" action="{{ url('updateContact', ['id' => $contact->id]) }}">
        @csrf
        @method('PUT')

    <div style="display: flex; align-items: center;">
    <h1 style="flex: 1; text-align: center; color: red; margin-left: 160px;">Edito {{ $contact->firstname }} {{ $contact->lastname }}</h1>
        <label style="margin-left: 10px;">
    Publish my Contact
    <input class="form-check-input" type="checkbox" name="publish" value="1" {{ $contact->publish == 1 ? 'checked' : '' }}>
    </label>
...
<table style="width:100%">
  ...
  <tr>
    <td>
      ...
      <input type="text" id="zip" name="zip"value="{{ $contact->zip }}"><br>
      <label for="country">Country:</label>
        <select name="country" id="country">
            <option value="UnitedKingdom" {{ $contact->country === 'UnitedKingdom' ? 'selected' : '' }}>UnitedKingdom</option>
            <option value="Albania" {{ $contact->country === 'Albania' ? 'selected' : '' }}>Albania</option>
            <option value="Kosova" {{ $contact->country === 'Kosova' ? 'selected' : '' }}>Kosova</option>
            <option value="UnitedStates" {{ $contact->country === 'UnitedStates' ? 'selected' : '' }}>UnitedStates</option>
        </select>
    </td>
    <td>
        <div id="phoneInputs">
